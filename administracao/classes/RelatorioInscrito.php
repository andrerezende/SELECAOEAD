...
class RelatorioInscrito {
	...
	public function mostrarAlunosPorCursos() {
		...
		$ssql .= " WHERE C.cod_curso in ( SELECT cod_curso FROM inscrito_curso )";
	...
	public function mostraralunosporlocalidadecurso() {
		...
		$ssql .= " where id in (select localprova from inscrito) ";
		//$ssql = $ssql . " order by nome ";
		...
		while ($linha = mysql_fetch_row($rs)) {
			...
			$ssql_2 .= " join inscrito_curso ic on ic.id_inscrito = i.id ";
			$ssql_2 .= " join curso c on c.cod_curso = ic.cod_curso ";
			$ssql_2 .= " where i.localprova = ".$linha[0];
			$ssql_2 .= " group by c.cod_curso,i.id ";
			$ssql_2 .= " order by c.cod_curso,trim(i.nome) asc ";
			...
			while ($linha_2 = mysql_fetch_row($rs_2)) {
				if ($curso != $linha_2[0]) {
					...
					$ssql_3 .= " join inscrito_curso ic on ic.id_inscrito = i.id ";
					$ssql_3 .= " where i.localprova = ".$linha[0];
					$ssql_3 .= " and ic.cod_curso = ".$linha_2[0];
